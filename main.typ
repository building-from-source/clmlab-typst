...
#[
  ...
  The main study compared the CLM tool condition with LLM-assisted interpretation of RStudio output.
  Participants received pre-fitted model output rather than being asked to generate and run an R script with an LLM.
  This allowed the study to focus on interpreting model output rather than participants' ability to use an LLM or the LLM's ability to generate a script.
  ...
  The live chatbot was not available in this condition.
  In the RStudio condition, participants interpreted model output with access to Google and ChatGPT during the permitted part of the task.
  ...
  The simple scenario concerned students' self-reported likelihood of applying in relation to parental education, institution type, and grade point average.
  The dataset, `ologit_data.csv`, contained 400 observations.
